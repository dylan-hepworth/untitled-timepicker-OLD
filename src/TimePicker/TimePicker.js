...
const TimePicker = ({
	...
	useEffect(() => {
		...
		// If a default time was provided, make sure that it matches the 24 hour format, otherwise check the format for a 12 hour format.
		if (defaultTime) {
			if (use24HourFormat) {
	...
	// Bold the selected time that was clicked, remove the bold from the other times, and scroll the selected time into view.
		...
		return [...Array(num)].map((e, i) => {
			...
			if ((i === 0 && type === "minute") || (i === 0 && use24HourFormat)) {
				...
				if (type === "minute" && incrementBy > 0) {
					if (i % incrementBy === 0) {
						return (
							<li key={i} onClick={clickEvent}>{(val.slice(-2) - 1)}</li>
						);
					}
				} else {
					return (
	...
	if (showError) {
		...
		return (
			<div>
				...
				<div className={`time_picker_wrapper ${(showDropdown && isOpen) ? "selection_open" : ""} ${compactMode ? "compact_mode" : ""} ${use24HourFormat ? "twenty_four_hour" : ""}`}>
					<div className="time_picker_input">
						...
						<input 
							...
							ref={minuteInputRef}
						/>
						{/* When not using the 24 hour format, show AM/PM */}
						{!use24HourFormat && (
							<input
								className="time_input"
								type="text"
